...
def primeGen(beginning, end):
    ...
    array1 = np.array(
        [range(beginning, end, 1)])  # creates a array of all values between beginning value and ending value

    array2 = np.delete(array1, np.where(
        array1 <= 1))  # filters all values equal to or smaller than 1 out, neither are prime numbers. if no such numbers exist both arrays are identical.
    ...
    while (arrayLocation <= len(array2)):  # iterates through every number in array irrespective of array length
        ...
        if (output[arrayLocation] == 1):  # checks current location to see if it could be a prime number. if not, skip
            ...
            for array1 in range(array2[arrayLocation] * array2[arrayLocation], end, array2[
                arrayLocation]):  # finds the whole range of non-primes originating from starter prime and puts them in a range.
...
def sortNumber(*tupleInputVal):
    ...
    while (processLocation < len(inputVal)):
        ...
        if inputVal[processLocation] != inputVal[
            smallestLocation]:  # make sure to not overwrite any data we don't want to
